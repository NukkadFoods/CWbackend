...
const emailTransporter = nodemailer.createTransport({
  ...
  secure: false,
  auth: {
    user: 'cryptoWealthauth@gmail.com',
    pass: 'meccbhdlhgmvvmbs'
  }
});
...
const verifyCaptcha = async (captchaToken) => {
  ...
  try {
    const response = await axios.post('https://www.google.com/recaptcha/api/siteverify', 
      `secret=6LeIxAcTAAAAAGG-vFI1TnRWxMZNFuojJ4WifJWe&response=${captchaToken}`, 
      {
...
const sendVerificationEmail = async (email, firstName, verificationToken) => {
  ...
  const mailOptions = {
    from: 'cryptoWealthauth@gmail.com',
    to: email,
...
app.get('/', (req, res) => {
  res.json({
    message: 'CryptoWealth Backend API v2.0',
    status: 'Running',
...
app.get('/api', (req, res) => {
  res.json({
    message: 'CryptoWealth API v2.0 - Advanced Email Verification',
    status: 'Active',
    features: [
      'Advanced CAPTCHA Protection',
...
app.post('/api/auth/login', async (req, res) => {
  try {
    ...
    console.log('🔑 Password received:', req.body.password ? '[PASSWORD PROVIDED]' : '[NO PASSWORD]');
    ...
    if (!user) {
      ...
      console.log('🔐 Verifying password for existing user:', user.email);
      console.log('🔑 Stored password hash length:', user.password ? user.password.length : 'NO HASH');
